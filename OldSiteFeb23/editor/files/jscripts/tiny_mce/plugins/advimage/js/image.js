	...
	fillFileList : function(id, l) {
		...
		if (l && l.length > 0) {
			...
			tinymce.each(l, function(o) {
				opt = new Option(o[0], o[1]);
				if (typeof o[2] != "undefined") {
					if (typeof optgroupArr[o[2]] == "undefined") {
						optgroup = document.createElement('optgroup');
						optgroup.label = o[2];
						optgroup.appendChild(opt);
						optgroupArr[o[2]] = optgroup;
					} else {
						optgroupArr[o[2]].appendChild(opt);
					}
				} else {
					lst.options[lst.options.length] = option;
				}
			});
			for (opt in optgroupArr) {
				lst.appendChild(optgroupArr[opt]);
			}
		} else
			dom.remove(dom.getParent(id, 'tr'));
	},
