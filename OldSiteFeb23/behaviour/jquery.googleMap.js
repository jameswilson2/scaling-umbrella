...
$.fn.googleMap = function(options) {
	...
	return this.each(function() {
		...
		if(o.directions) {
			var detailsHTML = '<div class="directionsDetails"><h2 class="nopad colour">Get Directions</h3><label>Your Postcode</label> <input class="postcode autoText" data-default="enter your postcode" /> ';
			if(o.directionsButton) {
				detailsHTML += '<input type="image" src="'+o.directionsButton+'" class="directionsSearch" value="Get Directions" /></div>';
			} else {
				detailsHTML += '<input type="button" value="&gt; Get Route" class="directionsSearch" /></div>';
			}
			var d;
			if(o.directionsselector != "") {
				d = $(o.directionsselector);
			} else {
				d = g;
			}
			d.append(detailsHTML);
			d.find('.directionsSearch').click(getDirections);
			if(o.directionsresults) {
				$(o.directionsresults).append('<div class="directionResults clear"></div>');
			} else {
				d.append('<div class="directionResults clear"></div>');
			}
			...
			directionsDisplay.setPanel(d.children(".directionResults")[0]);
		}
